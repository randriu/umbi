...
def bytes_to_vector(
    ...
    if chunk_ranges is None:
        ...
        return [bytes_to_primitive(chunk, value_type, little_endian) for chunk in chunks]

    return [bytes_to_primitive(data[start:end], value_type, little_endian) for start, end in chunk_ranges]
...
def vector_to_bytes(
    ...
    chunks = [primitive_to_bytes(item, value_type, little_endian) for item in vector]
    chunk_ranges = None
